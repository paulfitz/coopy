...
void login_page(void){
  ...
  if( g.zLogin==0 ){
    ...
    cgi_printf("<p>You are currently logged in as <b>%h</b></p>\n"
           "<p>To change your login to a different user, enter\n",g.zLogin);
  }
  cgi_printf("your user-id and password at the left and press the\n"
  ...
  if( zAnonPw ){
    ...
    cgi_printf("<input type=\"hidden\" name=\"cs\" value=\"%u\"/>\n"
...
void login_check_credentials(void){
  ...
  */
  ...
  if( g.fHttpTrace && g.zLogin ){
    _ssfossil_fprintf(stderr, "# login: [%s] with capabilities [%s]\n", g.zLogin, zCap);
  }
...
void login_anonymous_available(void){
  if( !g.okHistory &&
      db_exists("SELECT 1 FROM user"
                ...
                "   AND cap LIKE '%%h%%'") ){
    const char *zUrl = PD("REQUEST_URI", "index");
    cgi_printf("<p>Many <font color=\"red\">hyperlinks are disabled.</font><br />\n"
           "Use <a href=\"%s/login?anon=1&g=%T\">anonymous login</a>\n"

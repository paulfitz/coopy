...
void db_close(void){
  ...
  if (g.db) {
    if (sqlite3_close(g.db)!=SQLITE_OK) {
      _ssfossil_printf("Problem closing database (1)\n");
    }
    g.db = 0;
  }
  if (g.dbConfig) {
    if (sqlite3_close(g.dbConfig)!=SQLITE_OK) {
      _ssfossil_printf("Problem closing database (2)\n");
    }
    g.dbConfig = 0;
  }
}
...
void create_repository_cmd(void){
  ...
  _ssfossil_printf("project-id: %s\n", db_get("project-code", 0));
  _ssfossil_printf("server-id:  %s\n", db_get("server-code", 0));
  zPassword = db_text(0, "SELECT pw FROM user WHERE login=%Q", g.zLogin);
  _ssfossil_printf("admin-user: %s (initial password is \"%s\")\n", g.zLogin, zPassword);
}
...
static void db_sql_print(
  ...
  if( g.fSqlPrint ){
    for(i=0; i<argc; i++){
      char c = i==argc-1 ? '\n' : ' ';
      _ssfossil_printf("%s%c", sqlite3_value_text(argv[i]), c);
    }
  }
}
...
static void print_setting(const char *zName){
  ...
  if( db_step(&q)==SQLITE_ROW ){
    _ssfossil_printf("%-20s %-8s %s\n", zName, db_column_text(&q, 0),
        db_column_text(&q, 1));
  }else{
    _ssfossil_printf("%-20s\n", zName);
  }
  db_finalize(&q);
}

...
int main(int argc, char **argv){
  ...
  if( getenv("GATEWAY_INTERFACE")!=0 ){
    ...
    _ssfossil_fprintf(stderr, "Usage: %s COMMAND ...\n", argv[0]);
    _ssfossil_exit(1);
  }else{
  ...
  if( rc==1 ){
    _ssfossil_fprintf(stderr,"%s: unknown command: %s\n"
                   "%s: use \"help\" for more information\n",
                   argv[0], zCmdName, argv[0]);
    return 1;
  }else if( rc==2 ){
    _ssfossil_fprintf(stderr,"%s: ambiguous command prefix: %s\n"
                   "%s: use \"help\" for more information\n",
                   argv[0], zCmdName, argv[0]);
    return 1;
  }
...
void usage(const char *zFormat){
  _ssfossil_fprintf(stderr, "Usage: %s %s %s\n", g.argv[0], g.argv[1], zFormat);
  _ssfossil_exit(1);
}
...
static void multi_column_list(const char **azWord, int nWord){
  ...
  for(i=0; i<nRow; i++){
    ...
    for(j=i; j<nWord; j+=nRow){
      _ssfossil_printf("%s%-*s", zSpacer, mxLen, azWord[j]);
      zSpacer = "  ";
    }
    _ssfossil_printf("\n");
  }
...
void version_cmd(void){
  _ssfossil_printf("This is fossil version " MANIFEST_VERSION " " MANIFEST_DATE " UTC\n");
}
...
void help_cmd(void){
  ...
  const char *z;
  if( g.argc!=3 ){
    _ssfossil_printf("Usage: %s help COMMAND.\nAvailable COMMANDs:\n", g.argv[0]);
    cmd_cmd_list();
    version_cmd();
    return;
  }
...
void cmd_webserver(void){
  ...
  if( g.fHttpTrace || g.fSqlTrace ){
    _ssfossil_fprintf(stderr, "====== SERVER pid %d =======\n", getpid());
  }
  g.cgiPanic = 1;
  find_server_repository(isUiCmd);
  g.zRepositoryName = enter_chroot_jail(g.zRepositoryName);

...
static int find_area(const char *z){
  ...
  _ssfossil_printf("Available configuration areas:\n");
  for(i=0; i<count(aGroupName); i++){
    _ssfossil_printf("  %-10s %s\n", aGroupName[i].zName, aGroupName[i].zHelp);
  }
...
void configuration_cmd(void){
  ...
  if( strncmp(zMethod, "reset", n)==0 ){
    ...
    db_end_transaction(0);
    _ssfossil_printf("Configuration reset to factory defaults.\n");
    _ssfossil_printf("To recover, use:  %s %s import %s\n", 

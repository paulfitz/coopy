...
void client_sync(
  ...
  */
  ...
  manifest_crosslink_begin();
  _ssfossil_printf(zLabelFormat, "", "Bytes", "Cards", "Artifacts", "Deltas");
  ...
  while( go ){
    ...
    */
    ...
    _ssfossil_printf(zValueFormat, "Send:",
    ...
    */
    ...
    while( blob_line(&recv, &xfer.line) ){
      ...
      nCardRcvd++;
      if (!g.fQuiet) {
        _ssfossil_printf("\r%d", nCardRcvd);
        fflush(stdout);
      }
    ...
    origConfigRcvMask = 0;
    if( nCardRcvd>0 ){
      _ssfossil_printf(zValueFormat, "Received:",
  ...
  _ssfossil_printf("Total network traffic: %d bytes sent, %d bytes received\n",
         nSent, nRcvd);
  transport_close();

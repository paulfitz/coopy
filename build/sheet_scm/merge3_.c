...
int blob_merge(Blob *pPivot, Blob *pV1, Blob *pV2, Blob *pOut){
  ...
  int sub;
  static const char zBegin[] = ">>>>>>> BEGIN MERGE CONFLICT (ssfossil)\n";
  static const char zMid[]   = "============================\n";
  static const char zEnd[]   = "<<<<<<< END MERGE CONFLICT (ssfossil)\n";
...
void delta_3waymerge_cmd(void){
  ...
  if( g.argc!=6 ){
    _ssfossil_fprintf(stderr,"Usage: %s %s PIVOT V1 V2 MERGED\n", g.argv[0], g.argv[1]);
    _ssfossil_exit(1);
  }
  if( blob_read_from_file(&pivot, g.argv[2])<0 ){
    _ssfossil_fprintf(stderr,"cannot read %s\n", g.argv[2]);
    _ssfossil_exit(1);
  }
  ...
  if( blob_write_to_file(&merged, g.argv[5])<blob_size(&merged) ){
    _ssfossil_fprintf(stderr,"cannot write %s\n", g.argv[4]);
    _ssfossil_exit(1);
  }
  blob_reset(&pivot);
  blob_reset(&v1);

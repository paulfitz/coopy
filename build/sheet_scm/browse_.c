...
void page_dir(void){
  ...
  if( zCI ){
    ...
    zShort[10] = 0;
    cgi_printf("<h2>Files of check-in [<a href=\"vinfo?name=%T\">%s</a>]\n"
           "%s</h2>\n",zUuid,zShort,blob_str(&dirname));
  ...
  */
  ...
  cgi_printf("<table border=\"0\" width=\"100%%\"><tr><td valign=\"top\" width=\"25%%\">\n");
  i = 0;
  while( db_step(&q)==SQLITE_ROW ){
    const char *zFN;
    if( i==nRow ){
      cgi_printf("</td><td valign=\"top\" width=\"25%%\">\n");
      i = 0;
    }
    i++;
    ...
    if( zFN[0]=='/' ){
      zFN++;
      cgi_printf("<li><a href=\"%s%T\">%h/</a></li>\n",zSubdirLink,zFN,zFN);
    }else if( zCI ){
      const char *zUuid = db_column_text(&q, 1);
      cgi_printf("<li><a href=\"%s/artifact?name=%s\">%h</a>\n",g.zBaseURL,zUuid,zFN);
    }else{
      cgi_printf("<li><a href=\"%s/finfo?name=%T%T\">%h</a>\n",g.zBaseURL,zPrefix,zFN,zFN);
    }
  }
  db_finalize(&q);

...
void changes_cmd(void){
  ...
  _ssfossil_printf("%s",blob_buffer(&report));
}
...
void ls_cmd(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( isBrief ){
      ...
      _ssfossil_printf("ADDED      %s\n", zPathname);
    }else if( !file_isfile(zFullName) ){
      if( access(zFullName, 0)==0 ){
        _ssfossil_printf("NOT_A_FILE %s\n", zPathname);
      }else{
        _ssfossil_printf("MISSING    %s\n", zPathname);
      }
    }else if( isDeleted ){
      _ssfossil_printf("DELETED    %s\n", zPathname);
    }else if( chnged ){
      _ssfossil_printf("EDITED     %s\n", zPathname);
    }else if( renamed ){
      _ssfossil_printf("RENAMED    %s\n", zPathname);
    }else{
      _ssfossil_printf("UNCHANGED  %s\n", zPathname);
    }
    free(zFullName);
  }
  db_finalize(&q);
...
void commit_cmd(void){
  ...
  */
  ...
  _ssfossil_printf("New_Version: %s\n", zUuid);
  ...
  */
  ...
  if( count_nonbranch_children(vid)>1 ){
    _ssfossil_printf("**** warning: a fork has occurred *****\n");
  }
}
...
#define USAGE ("DATE COMMENT ?-p|-parent PARENT_RID...? ?-f|-file (FILE_RID PATH)...?")
  ...
  */
  ...
  _ssfossil_printf("inserted as record %d, %s\n", mid, zMidUuid);
  free(zMidUuid);
  return;

...
void view_edit(void){
  ...
  if( zErr ){
    cgi_printf("<blockquote><font color=\"#ff0000\"><b>%h</b></font></blockquote>\n",zErr);
  }
  cgi_printf("<form action=\"rptedit\" method=\"POST\">\n"
         "<input type=\"hidden\" name=\"rn\" value=\"%d\">\n"
         "<p>Report Title:<br>\n"
...
static void column_header(int rn,const char *zCol, int nCol, int nSorted,
  ...
  */
  if(set && desc){
    cgi_printf("<th bgcolor=\"%s\" class=\"bkgnd1\">\n"
           "  <a href=\"rptview?rn=%d%s\">%h</a></th>\n",BG1,rn,zExtra,zCol);
  }else{
    if(set){
      cgi_printf("<th bgcolor=\"%s\" class=\"bkgnd1\"><a\n",BG1);
    }else{
      cgi_printf("<th><a\n");
    }
    cgi_printf("href=\"rptview?rn=%d&amp;order_by=%d&amp;\\\n"
           "order_dir=%s\\\n"
           "%s\">%h</a></th>\n",rn,nCol,desc?"ASC":"DESC",zExtra,zCol);
  }
}
#endif
...
static int generate_html(
  ...
  */
  ...
  for(i=0; i<nArg; i++){
    ...
    if( pState->iNewRow>=0 && i>=pState->iNewRow ){
      if( zTid && g.okWrite ){
        cgi_printf("<td valign=\"top\"><a href=\"tktedit/%h\">edit</a></td>\n",zTid);
        zTid = 0;
      }
      if( zData[0] ){
        Blob content;
        cgi_printf("</tr><tr bgcolor=\"%h\"><td colspan=%d>\n",zBg,pState->nCol);
      ...
      zTid = zData;
      if( g.okHistory ){
        cgi_printf("<td valign=\"top\"><a href=\"tktview?name=%h\">%h</a></td>\n",zData,zData);
      }else{
        cgi_printf("<td valign=\"top\">%h</td>\n",zData);
      }
    }else if( zData[0]==0 ){

...
void tktnew_page(void){
  ...
  cgi_printf("<form method=\"POST\" action=\"%s/%s\">\n",g.zBaseURL,g.zPath);
...
void tktedit_page(void){
  ...
  if( zName==0 || (nName = strlen(zName))<4 || nName>UUID_SIZE
    ...
    cgi_printf("<font color=\"red\"><b>Not a valid ticket id: \\\"%h\\\"</b></font>\n",zName);
    style_footer();
    return;
  }
  nRec = db_int(0, "SELECT count(*) FROM ticket WHERE tkt_uuid GLOB '%q*'",
                zName);
  if( nRec==0 ){
    cgi_printf("<font color=\"red\"><b>No such ticket: \\\"%h\\\"</b></font>\n",zName);
    style_footer();
    return;
  }
  if( nRec>1 ){
    cgi_printf("<font color=\"red\"><b>%d tickets begin with: \\\"%h\\\"</b></font>\n",nRec,zName);
    style_footer();
    return;
  }
...
void tkthistory_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( manifest_parse(&m, &content) && m.type==CFTYPE_TICKET ){
      ...
      cgi_printf("\n"
             "Ticket change\n"
             "[<a href=\"%s/artifact/%T\">%s</a>]</a>\n"
             "(rid %d) by\n",g.zTop,zChngUuid,zUuid,rid);
...
void ticket_output_change_artifact(Manifest *pTkt){
  ...
  for(i=0; i<pTkt->nField; i++){
    ...
    if( z[0]=='+' ){
      cgi_printf("<li>Appended to %h:<blockquote>\n",&z[1]);
      wiki_convert(&val, 0, 0);
      cgi_printf("</blockquote></li>\n");
    }else if( blob_size(&val)<=50 && contains_newline(&val) ){
      cgi_printf("<li>Change %h to:<blockquote>\n",z);
      wiki_convert(&val, 0, 0);
      cgi_printf("</blockquote></li>\n");
    }else{

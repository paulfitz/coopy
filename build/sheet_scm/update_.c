  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( idv>0 && ridv==0 && idt>0 && ridt>0 ){
      ...
      */
      _ssfossil_printf("CONFLICT %s\n", zName);
    }else if( idt>0 && idv==0 ){
      /* File added in the target. */
      _ssfossil_printf("ADD %s\n", zName);
      undo_save(zName);
      if( !nochangeFlag ) vfile_to_disk(0, idt, 0);
    }else if( idt>0 && idv>0 && ridt!=ridv && chnged==0 ){
      /* The file is unedited.  Change it to the target version */
      _ssfossil_printf("UPDATE %s\n", zName);
      undo_save(zName);
      if( !nochangeFlag ) vfile_to_disk(0, idt, 0);
      ...
      ** version that appears in the target. */
      ...
      if( rc>=0 ){
        ...
        if( rc>0 ){
          _ssfossil_printf("***** %d merge conflicts in %s\n", rc, zName);
        }
      }else{
        _ssfossil_printf("***** Cannot merge binary file %s\n", zName);
      }
      blob_reset(&v);
      blob_reset(&e);
      blob_reset(&t);
...
void revert_cmd(void){
  ...
  undo_finish();
  db_end_transaction(0);
  _ssfossil_printf("\"fossil undo\" is available to undo the changes shown above.\n");
}

...
static int check_name(const char *z){
  if( !wiki_name_is_wellformed((const unsigned char *)z) ){
    ...
    cgi_printf("The wiki name \"<b>%h</b>\" is not well-formed.  Rules for\n"
           "wiki page names:\n",z);
...
void home_page(void){
  ...
  cgi_printf("<p>This is a stub home-page for the project.\n"
         ...
         "<a href=\"%s/setup_config\">setup/config</a>\n"
...
void wiki_page(void){
  ...
  if( zPageName==0 ){
    ...
    { char *zHomePageName = db_get("project-name",0);
      if( zHomePageName ){
        cgi_printf("<li> <a href=\"%s/wiki?name=%t\">\n"
               "     %h</a> wiki home page.</li>\n",g.zBaseURL,zHomePageName,zHomePageName);
      }
    }
    cgi_printf("<li> <a href=\"%s/timeline?y=w\">Recent changes</a> to wiki\n"
           "     pages. </li>\n"
           "<li> <a href=\"%s/wiki_rules\">Formatting rules</a> for \n"
           "     wiki.</li>\n"
           "<li> Use the <a href=\"%s/wiki?name=Sandbox\">Sandbox</a>\n"
           "     to experiment.</li>\n",g.zBaseURL,g.zBaseURL,g.zBaseURL);
    if( g.okNewWiki ){
      cgi_printf("<li>  Create a <a href=\"%s/wikinew\">new wiki page</a>.</li>\n",g.zBaseURL);
    }
    cgi_printf("<li> <a href=\"%s/wcontent\">List of All Wiki Pages</a>\n"
           "     available on this server.</li>\n"
        "<li> <form method=\"GET\" action=\"%s/wfind\">\n"
...
void wikiedit_page(void){
  ...
  if( n<20 ) n = 20;
  if( n>40 ) n = 40;
  cgi_printf("<form method=\"POST\" action=\"%s/wikiedit\">\n",g.zBaseURL);
...
void wikinew_page(void){
  ...
  cgi_printf("<p>Rules for wiki page names:\n");
  well_formed_wiki_name_rules();
  cgi_printf("</p>\n"
         "<form method=\"POST\" action=\"%s/wikinew\">\n"
         "<p>Name of new wiki page:\n"
         "<input type=\"text\" width=\"35\" name=\"name\" value=\"%h\">\n"
         "<input type=\"submit\" value=\"Create\">\n"
         "</p></form>\n",g.zBaseURL,zName);
  if( zName[0] ){
    cgi_printf("<p><b><font color=\"red\">\n"
           "\"%h\" is not a valid wiki page name!</font></b></p>\n",zName);
  }
  style_footer();
}
...
void wikiappend_page(void){
  ...
  zUser = PD("u", g.zLogin);
  cgi_printf("<form method=\"POST\" action=\"%s/wikiappend\">\n",g.zBaseURL);
  login_insert_csrf_secret();
  cgi_printf("<input type=\"hidden\" name=\"name\" value=\"%h\">\n"
         "Your Name:\n"
...
static void wiki_history_extra(int rid){
  cgi_printf("<a href=\"%s/wdiff?name=%t&a=%d\">[diff]</a>\n",g.zTop,zWikiPageName,rid);
}
...
void wcontent_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<li><a href=\"%s/wiki?name=%T\">%h</a></li>\n",g.zBaseURL,zName,zName);
  }
  db_finalize(&q);
  cgi_printf("</ul>\n");
  style_footer();
...
void wiki_cmd(void){
  ...
  if( strncmp(g.argv[2],"commit",n)==0
    ...
    if( g.argv[2][1]=='r' ){
      ...
      _ssfossil_printf("Created new wiki page %s.\n", zPageName);
    }else{
      wiki_cmd_commit(zPageName, 0, &content);
      _ssfossil_printf("Updated wiki page %s.\n", zPageName);
    }
    blob_reset(&content);
  }else
